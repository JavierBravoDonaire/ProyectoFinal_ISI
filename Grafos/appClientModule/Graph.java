...
public class Graph {
   ...
   /**
    ...
    /**
     * Calculate the importance of a vertex in the graph.
     * The importance is an absolute number.
     * 
     * @param v the vertex
     * @return the importance (popularity) of vertex v in this graph
     */
    public double popularity(String v) {
    	validateVertex(v);
    	int pop1 = degree(v); // popularity determined by films
    	double pop2 = 0.0;    // popularity determined by collaborations
    						  // (mean of popularity from collaboration actors)
    	int count = 0;
    	for (String film : adjacentTo(v)) {
            for (String actor : adjacentTo(film)) {
            	if (actor.equals(v)) continue; // it's the same actor
            	pop2 = degree(actor) + pop2;
            	count++;
            }
        }
    	pop2 = pop2/(double)count;
    	return ((double)pop1 + pop2) / 2; // return the mean of two popularity
    }
