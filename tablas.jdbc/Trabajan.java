...
public class Trabajan {
								...
								ClassNotFoundException, SQLException
	{
		...
		get("/:table/:film_id", Trabajan::doSelect);

		// In this case we use a Java 8 Lambda function to process the
		// GET /upload_films HTTP request, and we return a form
		get("/upload_works", (req, res) ->
		    "<form action='/upload_works' method='post' enctype='multipart/form-data'>"
		    + "    <input type='file' name='uploaded_works_file' accept='.txt'>"
		...
		post("/upload_works", (req, res) -> {
			req.attribute("org.eclipse.jetty.multipartConfig", new MultipartConfigElement("/tmp"));
			String result = "File uploaded!";
			try (InputStream input = req.raw().getPart("uploaded_works_file").getInputStream()) {
